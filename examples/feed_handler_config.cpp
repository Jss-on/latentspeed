/**
 * @file feed_handler_config.cpp
 * @brief Config-based feed handler similar to marketstream
...
// Callback to log market data
class ConfigCallback : public MarketDataCallbacks {
public:
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        auto logger = std::make_shared<spdlog::logger>("console", begin(sinks), end(sinks));
        ...
        spdlog::info("===========================================");
        spdlog::info("LatentSpeed Feed Handler (Config Mode)");
        ...
        auto callbacks = std::make_shared<ConfigCallback>();
        ...
        for (const auto& feed_node : config["feeds"]) {
            ...
            spdlog::info("Adding {} feed with {} symbols", 
        ...
        // Start all feeds
        spdlog::info("Starting all feeds...");
        g_feed_handler->start();
        
        spdlog::info("===========================================");
        spdlog::info("Streaming data (Press Ctrl+C to stop)");
        if (config["zmq"] && config["zmq"]["enabled"].as<bool>(true)) {
            spdlog::info("ZMQ Ports: {} (trades), {} (orderbooks)", 
                        feed_config.zmq_trades_port, feed_config.zmq_books_port);
        }
        spdlog::info("===========================================\n");
        
        // Stats loop
        auto start_time = std::chrono::steady_clock::now();
        int stats_interval = 10; // seconds
